...
3. **CLI ctx.obj**（由 `vitriol.cli.main` 注入的全局开关）
...
- 目标：提供 UI 开关（例如 checkbox：`trust_remote_code`），默认值可从环境变量读取（例如 `VITRIOL_UI_TRUST_REMOTE_CODE`）或延用 True
...
#### 4.1.3 用户可见行为
- CLI：`vitriol --no-trust-remote-code analyze ...` / `vitriol --no-trust-remote-code evolve ...` 实际生效
...
- `output_dir?: str`（可选，默认为 `<VITRIOL_API_OUTPUT_DIR>/nas/<job_id>`；若未设置环境变量则使用 `./api_output/nas/<job_id>`）
...
#### 4.3.1 返回内容定位
`/models` 的目标不是“列出所有 HF 模型”，而是返回 Vitriol 内置的：
...
#### 4.3.2 数据来源（可组合）
优先从以下模块汇总：
- `vitriol.evolution.tree_builder.DEFAULT_FAMILIES`
- `vitriol.compat.family_matrix`（如果能提供更工程化的矩阵）
- `vitriol.adapters.registry.AdapterRegistry`（用于列 adapter module 清单/可用性）
