...
## 能力分层（Stable / Beta / Experimental）

Vitriol 功能面较广，下表说明各模块的**稳定性承诺**，便于选型与集成：

| 层级 | CLI / 模块 | 承诺 |
|------|------------|------|
| **Stable** | `check`, `generate`, `validate`, `analyze`, `arch-viz`, `viz`, `hash`, `evolve`（tree/compare/simulate） | 主线工作流；semver 内保持 CLI 契约 |
| **Beta** | `nas`（targeted/evolutionary）, `bench kv-*`, CIS 评分 | 可用，接口可能微调 |
| **Experimental** | `exobrain`, `infer`, `trace`, `bench turboquantum`, RL NAS, `webui`, REST API | 标注 `@experimental`，不保证 API 稳定 |

**推荐入口**：[`vitriol check`](docs/case-studies/01-zero-download-architecture-compare.md) — 一条命令跑通 analyze → viz → generate → validate → 报告。  
**CI 集成**：见 [Case Study 02](docs/case-studies/02-ci-model-config-validation.md) 与 [CI 集成指南](docs/ci-integration.md)。  
**CIS 策略排行**：见 [Case Study 03](docs/case-studies/03-cis-strategy-ranking.md) 与 `vitriol cis rank`。  
**多策略对比**：见 [Case Study 04](docs/case-studies/04-multi-strategy-cis-validate-compare.md) 与 `vitriol cis compare`。
...
- **19 个 CLI 命令**：从 `check` 一键闭环到基准测试和推理的完整工具链
